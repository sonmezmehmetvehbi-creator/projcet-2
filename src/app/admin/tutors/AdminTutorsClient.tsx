...
export default function AdminTutorsClient({ applications: initialApps }: Props) {
  ...
  const statusColor = (s: string) => s === 'approved' ? 'rgb(34,85,14)' : s === 'rejected' ? 'rgb(248,113,113)' : 'rgb(180,120,10)'
    ...
    <div className="animate-fade-in" style={{ paddingTop: '5rem', minHeight: '100vh' }}>
      <div style={{ maxWidth: '72rem', margin: '0 auto', padding: '2rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: '2rem', fontWeight: 700, color: 'rgb(240,240,235)', marginBottom: '0.25rem' }}>
              Tutor Applications
            </h1>
            <p style={{ color: 'rgba(255,255,255,0.55)' }}>{applications.filter(a => a.status === 'pending').length} pending review</p>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            {(['all', 'pending', 'approved', 'rejected'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                style={{ padding: '0.5rem 1rem', borderRadius: '0.75rem', border: `1.5px solid ${filter === f ? 'rgb(34,85,14)' : 'rgba(34,85,14,0.2)'}`, background: filter === f ? 'rgba(34,85,14,0.08)' : 'transparent', color: filter === f ? 'rgb(34,85,14)' : 'rgba(255,255,255,0.55)', fontWeight: filter === f ? 600 : 400, fontSize: '0.875rem', cursor: 'pointer', textTransform: 'capitalize' }}>
        ...
        {filtered.length === 0 && (
          <div className="card" style={{ padding: '3rem', textAlign: 'center', color: 'rgba(255,255,255,0.55)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {filtered.map(app => (
            <div key={app.id} className="card" style={{ padding: '1.5rem', border: app.status === 'pending' ? '2px solid rgba(232,160,32,0.2)' : '1px solid rgba(255,255,255,0.08)' }}>

              {/* Header */}
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.375rem', flexWrap: 'wrap' }}>
                    <h3 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: '1.25rem', fontWeight: 700, color: 'rgb(240,240,235)' }}>
                      {app.display_name}
                    </h3>
                  ...
                  <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.55)', marginBottom: '0.5rem' }}>{app.profiles?.email}</p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem', marginBottom: '0.5rem' }}>
                    {app.subjects?.map((s: string) => (
                      <span key={s} style={{ fontSize: '0.75rem', padding: '0.2rem 0.5rem', borderRadius: '9999px', background: 'rgba(34,85,14,0.06)', color: 'rgb(122,192,74)', fontWeight: 600 }}>{s}</span>
                    ))}
                  </div>
                  <div style={{ display: 'flex', gap: '1rem', fontSize: '0.8125rem', color: 'rgba(255,255,255,0.55)', flexWrap: 'wrap' }}>
              ...
              {selected === app.id && (
                <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px,1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
                    ...
                    {/* Personal Info */}
                    <div style={{ padding: '1.25rem', borderRadius: '0.875rem', background: 'rgba(34,85,14,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'rgb(122,192,74)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.875rem' }}>👤 Personal Info</p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        { label: 'LinkedIn', value: app.linkedin_url },
                          ...
                          <div key={item.label}>
                            <p style={{ fontSize: '0.6875rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</p>
                            <p style={{ fontSize: '0.875rem', color: 'rgb(240,240,235)', lineHeight: 1.6 }}>{item.value}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div style={{ padding: '1.25rem', borderRadius: '0.875rem', background: 'rgba(37,99,235,0.03)', border: '1px solid rgba(37,99,235,0.1)' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <div>
                          <p style={{ fontSize: '0.6875rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Subjects</p>
                        ...
                        <div>
                          <p style={{ fontSize: '0.6875rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Hourly Rate</p>
                          <p style={{ fontSize: '0.875rem', color: 'rgb(240,240,235)' }}>${app.hourly_rate}/hr</p>
                        </div>
                        <div>
                          <p style={{ fontSize: '0.6875rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Applied</p>
                          <p style={{ fontSize: '0.875rem', color: 'rgb(240,240,235)' }}>{new Date(app.created_at).toLocaleString()}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Payment Info */}
                    <div style={{ padding: '1.25rem', borderRadius: '0.875rem', background: 'rgba(34,85,14,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'rgb(122,192,74)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.875rem' }}>💰 Payment Info</p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        {[
                          ...
                          <div key={item.label}>
                            <p style={{ fontSize: '0.6875rem', fontWeight: 700, color: 'rgba(255,255,255,0.55)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</p>
                            <p style={{ fontSize: '0.875rem', color: 'rgb(240,240,235)' }}>{item.value}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                        {app.cv_url && (
                          <a href={app.cv_url} target="_blank" rel="noopener noreferrer"
                            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.625rem 0.875rem', borderRadius: '0.625rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(180,120,10,0.2)', color: 'rgb(180,120,10)', textDecoration: 'none', fontSize: '0.875rem', fontWeight: 600 }}>
                            📄 View CV / Resume
                          </a>
                        )}
                        {app.certificate_url && (
                        ...
                        {app.id_verified !== undefined && (
                          <div style={{ padding: '0.5rem 0.875rem', borderRadius: '0.625rem', background: app.id_verified ? 'rgba(34,85,14,0.06)' : 'rgba(163,45,45,0.06)', border: `1px solid ${app.id_verified ? 'rgba(34,85,14,0.2)' : 'rgba(163,45,45,0.2)'}` }}>
                            <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: app.id_verified ? 'rgb(34,85,14)' : 'rgb(248,113,113)' }}>
                  ...
                  {/* Decision */}
                  {app.status === 'pending' && (
                    <div style={{ padding: '1.5rem', borderRadius: '0.875rem', background: 'rgba(34,85,14,0.02)', border: '1px solid rgba(255,255,255,0.08)' }}>
                      <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontWeight: 700, color: 'rgb(240,240,235)', marginBottom: '1rem', fontSize: '1rem' }}>
                        Make a Decision
                      </p>
                      <div>
                        <label className="label">Note to tutor <span style={{ fontWeight: 400, color: 'rgba(255,255,255,0.55)' }}>(optional — included in email)</span></label>
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        ...
                        <button onClick={() => updateStatus(app.id, 'rejected', app.user_id, app.profiles?.email, app.display_name)}
                          disabled={loading === app.id}
                          style={{ flex: 1, minWidth: '140px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.875rem 1.5rem', borderRadius: '0.875rem', background: 'rgba(163,45,45,0.08)', border: '2px solid rgba(163,45,45,0.3)', color: 'rgb(248,113,113)', fontWeight: 700, fontSize: '0.9375rem', cursor: 'pointer', transition: 'all 0.2s' }}>
