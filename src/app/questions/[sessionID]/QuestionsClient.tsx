...
import { CheckCircle, XCircle, ArrowLeft, ArrowRight, RotateCcw, Download, Home } from 'lucide-react'
...
export default function QuestionsClient({ session }: Props) {
  ...
  if (showSummary) return <Summary questions={questions} answers={answers} score={score} total={total} session={session} onRestart={() => { setAnswers({}); setFrInputs({}); setFrFeedback({}); setCurrent(0); setShowSummary(false) }} />
    ...
    <div style={{ paddingTop:'5rem', minHeight:'100vh' }}>
      <div className="container-base" style={{ padding:'2rem 1.5rem', maxWidth:'48rem' }}>
        ...
        <div className="card" style={{ padding:'2rem', marginBottom:'1.5rem' }}>
          ...
          {q.type === 'mc' ? (
            <MCOptions question={q} answered={answers[current]} onSelect={(c) => selectMC(q as MCQuestion, c)} />
          ) : (
            <FRInput
...
function MCOptions({ question, answered, onSelect }: { question: MCQuestion; answered?: { answer: string; correct: boolean | null }; onSelect: (c: string) => void }) {
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
      {question.options.map(option => {
        ...
        return (
          <button key={letter} onClick={() => onSelect(letter)} className={cls} style={{ width:'100%', background:'none', border: undefined, textAlign:'left' }}>
            <div style={{ width:'1.75rem', height:'1.75rem', borderRadius:'50%', border:`2px solid ${answered ? (isCorrect ? 'rgb(59,109,17)' : isSelected ? 'rgb(163,45,45)' : 'rgba(34,85,14,0.2)') : 'rgba(34,85,14,0.3)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, fontSize:'0.8125rem', fontWeight:700, color: answered ? (isCorrect ? 'rgb(59,109,17)' : isSelected ? 'rgb(163,45,45)' : 'rgb(107,107,88)') : 'rgb(34,85,14)' }}>
              {letter}
            </div>
            <span style={{ fontSize:'0.9375rem', color:'rgb(26,26,20)', lineHeight:1.5 }}>{option.substring(3)}</span>
      ...
      {/* Feedback */}
      {answered && (
        <div style={{ marginTop:'0.5rem', padding:'1.25rem', borderRadius:'0.875rem', background: answered.correct ? 'rgb(234,243,222)' : 'rgb(252,235,235)', border:`1px solid ${answered.correct ? 'rgba(59,109,17,0.2)' : 'rgba(163,45,45,0.2)'}` }}>
          <p style={{ fontWeight:700, fontSize:'1rem', color: answered.correct ? 'rgb(59,109,17)' : 'rgb(163,45,45)', marginBottom:'0.5rem' }}>
            {answered.correct ? '🎉 Excellent!!! Great job!' : '💡 Not quite — here\'s how to think about it:'}
          </p>
          <p style={{ fontSize:'0.9375rem', color: answered.correct ? 'rgba(59,109,17,0.9)' : 'rgba(163,45,45,0.9)', lineHeight:1.7 }}>
...
function FRInput({ question, value, onChange, onSubmit, loading, feedback, answered }: {
  ...
  onSubmit: () => void; loading: boolean; feedback?: { score: string; feedback: string }; answered: boolean
}) {
  return (
          ...
          {questions.map((q, i) => {
            ...
            return (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.75rem 1rem', borderRadius:'0.75rem', background:'white', border:'1px solid rgba(34,85,14,0.08)' }}>
                ...
                <span className={`badge ${correct ? 'badge-success' : ''}`} style={{ background: !answered ? 'rgba(34,85,14,0.05)' : undefined, color: !answered ? 'rgb(107,107,88)' : undefined, flexShrink:0 }}>
                  {!answered ? 'Skipped' : correct ? 'Correct' : 'Review'}
                </span>
              </div>
            )
          })}
