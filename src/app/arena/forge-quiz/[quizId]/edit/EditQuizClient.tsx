...
import {
  QuestionEditor, PublicToggle, newQuestion, isValid,
  type Question, type QType,
...
export default function EditQuizClient({ quiz, initialQuestions }: { quiz: any; initialQuestions: any[] }) {
  ...
  // Save the edited questions back to this quiz, then open the unified launch
  // chooser (Play Solo / Self-Paced Room / Host Live) which relaunches a copy.
  async function saveThenLaunch() {
  ...
  return (
    <div style={{ maxWidth: '46rem', margin: '0 auto', padding: '6rem 1.5rem 4rem' }}>
      <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: '2rem', fontWeight: 800, color: 'white', marginBottom: '0.375rem' }}>✏️ Edit Quiz</h1>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {error && <p style={{ margin: '0.25rem 0 0', color: 'rgb(248,113,113)', fontSize: '0.8125rem' }}>{error}</p>}
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '1rem', marginTop: '0.5rem' }}>
          <button type="button" onClick={() => router.push('/arena')}
            ...
            <ArrowLeft style={{ width: '1rem', height: '1rem' }} /> Back to Arena
          </button>
          <button type="button" onClick={() => validQuestions.length >= 1 && saveThenLaunch()} disabled={busy || validQuestions.length < 1}
            style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', height: '3rem', padding: '0 1.75rem', borderRadius: '0.875rem', border: 'none', background: validQuestions.length >= 1 ? 'linear-gradient(90deg, rgb(124,58,237), rgb(139,92,246))' : 'rgba(255,255,255,0.06)', color: validQuestions.length >= 1 ? 'white' : 'rgb(120,120,140)', fontWeight: 800, cursor: busy ? 'wait' : validQuestions.length >= 1 ? 'pointer' : 'not-allowed' }}>
            {busy ? <><Loader2 style={{ width: '1rem', height: '1rem' }} className="animate-spin" /> Saving…</> : <>Save &amp; Launch <ArrowRight style={{ width: '1rem', height: '1rem' }} /></>}
          </button>
        </div>
      </div>

      {launchOpen && <LaunchChooser quizId={quiz.id} onClose={() => setLaunchOpen(false)} owner title="Launch quiz" />}
