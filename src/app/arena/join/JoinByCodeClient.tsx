...
// Guest-friendly code entry. Checks the Live-session code space first, then
// Self-Paced rooms (same order as the Arena hub's join card), and routes to the
// matching join flow — all of which already accept guests via a guest_id.
export default function JoinByCodeClient() {
  ...
  async function submit() {
    ...
    try {
      // 1) Live Host session?
      const liveRes = await fetch(`/api/arena/forge-quiz/live/find-session?code=${encodeURIComponent(c)}`)
      ...
      // 2) Self-Paced Room?
      const res = await fetch(`/api/arena/forge-quiz/find-by-code?code=${encodeURIComponent(c)}`)
      const data = await res.json().catch(() => ({}))
      if (!res.ok || !data.quizId) throw new Error(data.error || 'Game not found or already started')
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', textAlign: 'center' }}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: '30rem' }}>
        ...
        <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: '2rem', fontWeight: 800, color: 'white', marginBottom: '0.5rem' }}>Join a Game</h1>
        <p style={{ color: 'rgb(148,148,168)', marginBottom: '2rem' }}>Enter the code from your host — no account needed.</p>
