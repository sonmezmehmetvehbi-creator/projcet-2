...
import { LogOut, ChevronDown, GraduationCap, FileText, Headphones } from 'lucide-react'
    ...
    <nav style={{
      ...
      <div style={{ maxWidth: '80rem', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '4rem', padding: '0 1.5rem', gap: '1rem' }}>
        ...
        <Link href="/tutor/dashboard" style={{ display: 'flex', alignItems: 'center', gap: '0.625rem', textDecoration: 'none', flexShrink: 0 }}>
          ...
          <span style={{ fontSize: '0.625rem', fontWeight: 800, padding: '0.2rem 0.5rem', borderRadius: '9999px', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white', fontFamily: 'Syne, sans-serif', letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            TUTOR
          </span>
        </Link>
          ...
          {NAV_LINKS.map(link => {
            ...
            return (
              <Link key={link.href} href={link.href}
                style={{ padding: '0.5rem 0.875rem', borderRadius: '0.625rem', fontSize: '0.875rem', fontWeight: isActive ? 600 : 400, color: isActive ? 'white' : 'rgba(255,255,255,0.5)', textDecoration: 'none', transition: 'all 0.2s', whiteSpace: 'nowrap', background: isActive ? 'rgba(99,102,241,0.2)' : 'transparent' }}
        ...
        <div style={{ position: 'relative', flexShrink: 0 }} ref={ref}>
          <button onClick={() => setOpen(o => !o)}
            ...
            <div style={{ width: '1.875rem', height: '1.875rem', borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.8125rem', fontWeight: 700 }}>
              {profile?.display_name?.[0] ?? 'T'}
            </div>
            <div style={{ textAlign: 'left' }}>
              <p style={{ fontSize: '0.8125rem', fontWeight: 600, color: 'white', lineHeight: 1.2 }}>{profile?.display_name ?? 'Tutor'}</p>
              <p style={{ fontSize: '0.625rem', color: 'rgba(255,255,255,0.4)', lineHeight: 1.2, fontFamily: 'Syne, sans-serif', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
          ...
          {open && (
            <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 0.5rem)', width: '15rem', background: 'rgb(18,18,30)', borderRadius: '0.875rem', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', zIndex: 100 }}>
