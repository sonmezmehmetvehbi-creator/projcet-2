...
export default function SupportClient({ profile, tickets: initialTickets }: Props) {
  ...
  async function createTicket() {
    if (!subject.trim() || !firstMessage.trim()) { setError('Please fill in both fields.'); return }
    setCreating(true)
    setError('')
    try {
    ...
    <div style={{ paddingTop:'5rem', minHeight:'100vh', paddingBottom:'4rem' }}>
      <div style={{ maxWidth:'64rem', margin:'0 auto', padding:'2rem 1.5rem' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'2rem', flexWrap:'wrap', gap:'1rem' }}>
          <div>
            <h1 style={{ fontFamily:'Fraunces, Georgia, serif', fontSize:'2rem', fontWeight:700, color:'rgb(26,26,20)', marginBottom:'0.25rem' }}>Support 💬</h1>
          ...
          <div className="card" style={{ padding:'1.5rem', marginBottom:'1.5rem', border:'2px solid rgba(34,85,14,0.2)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'0.875rem' }}>
              <div>
                ...
                <input value={subject} onChange={e => setSubject(e.target.value)} className="input" placeholder="e.g. Issue with my subscription, Question about tutoring..." />
              </div>
              <div>
                <label className="label">Describe your issue *</label>
                <textarea value={firstMessage} onChange={e => setFirstMessage(e.target.value)} className="input" rows={4} style={{ resize:'vertical' }} placeholder="Tell us what is going on and we will help you out..." />
              </div>
        ...
        {tickets.length === 0 && !showNewTicket ? (
          <div className="card" style={{ padding:'4rem', textAlign:'center' }}>
            ...
            <h2 style={{ fontFamily:'Fraunces, Georgia, serif', fontSize:'1.5rem', fontWeight:700, color:'rgb(26,26,20)', marginBottom:'0.75rem' }}>No tickets yet</h2>
            <p style={{ color:'rgb(107,107,88)', marginBottom:'1.5rem' }}>Have a question or issue? Open a support ticket and we will help you out.</p>
                  ...
                  <div style={{ flex:1, overflowY:'auto', padding:'1.5rem', display:'flex', flexDirection:'column', gap:'1rem' }}>
                    ...
                    {messages.map(msg => (
                      <div key={msg.id} style={{ display:'flex', flexDirection:'column', alignItems: msg.is_admin ? 'flex-start' : 'flex-end' }}>
                        ...
                        <p style={{ fontSize:'0.6875rem', color:'rgb(107,107,88)', marginTop:'0.25rem', paddingRight:'0.25rem' }}>
