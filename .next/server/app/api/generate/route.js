"use strict";(()=>{var e={};e.id=8290,e.ids=[8290],e.modules={72934:e=>{e.exports=require("next/dist/client/components/action-async-storage.external.js")},54580:e=>{e.exports=require("next/dist/client/components/request-async-storage.external.js")},45869:e=>{e.exports=require("next/dist/client/components/static-generation-async-storage.external.js")},20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},92048:e=>{e.exports=require("fs")},32615:e=>{e.exports=require("http")},35240:e=>{e.exports=require("https")},55315:e=>{e.exports=require("path")},68621:e=>{e.exports=require("punycode")},76162:e=>{e.exports=require("stream")},17360:e=>{e.exports=require("url")},21764:e=>{e.exports=require("util")},6162:e=>{e.exports=require("worker_threads")},71568:e=>{e.exports=require("zlib")},87561:e=>{e.exports=require("node:fs")},84492:e=>{e.exports=require("node:stream")},72477:e=>{e.exports=require("node:stream/web")},16880:(e,t,s)=>{s.r(t),s.d(t,{originalPathname:()=>w,patchFetch:()=>v,requestAsyncStorage:()=>g,routeModule:()=>h,serverHooks:()=>y,staticGenerationAsyncStorage:()=>f});var i={};s.r(i),s.d(i,{POST:()=>m});var o=s(49303),r=s(88716),n=s(60670),a=s(6943),c=s(3370),u=s(26830),l=s(57100),p=s(87070),d=s(54214);async function m(e){let t=new d.ZP({apiKey:process.env.OPENAI_API_KEY});try{let s;let i=await (0,a.f)(),{data:{user:o}}=await i.auth.getUser();if(!o)return p.NextResponse.json({error:"Unauthorized"},{status:401});let r=(0,c.createClient)(process.env.NEXT_PUBLIC_SUPABASE_URL,process.env.SUPABASE_SERVICE_ROLE_KEY),n=await (0,l.h)(r,o.id,"generation");if(n)return p.NextResponse.json({error:"generation_banned",reason:n.reason},{status:403});let{subject:d,grade:m,topic:h,focus:g,outputType:f,questionCount:y,questionTypes:w,uploadedText:v,isRetry:b,difficulty:q}=await e.json(),x="questions"===f?"questions":"worksheets",A=await (0,u.$o)(i,o.id,x);if(!A.allowed)return p.NextResponse.json({error:"Daily limit reached",limitReached:!0,isPremium:A.isPremium,bonusRemaining:A.bonus},{status:429});let k={mathematics:`
...
- Distractors: plausible-sounding but factually incorrect locations, statistics, or cause-effect relationships.`},C=d?.toLowerCase().trim(),T=["math","mathematics","calculus","algebra","pre-calculus","statistics","geometry","trigonometry"].some(e=>C?.includes(e)),S=k[C]||(T?k.mathematics:""),E=q||"medium",D=`You are AceForge, an expert educational content creator with deep knowledge of standardized exams, curriculum standards, and pedagogical best practices.
...
- Distractors are plausible but unambiguously wrong.`,F=v?`
...
${v}
---
`:"",P=w?.includes("mc"),R=w?.includes("fr"),O="";O="questions"===f?`Generate ${y} ${E.toUpperCase()} difficulty study questions about "${h}" in ${d} for a ${m} student.${g?` Focus specifically on: ${g}.`:""}${F}

Include ${P&&R?"a mix of multiple choice and free response":R?"ONLY free response — do not include any multiple choice questions":"ONLY multiple choice — do not include any free response questions"} questions. This is strictly enforced — if the type says ONLY, do not include any other type.
...
Ensure questions span different aspects of "${h}" — do not repeat the same concept.

Return JSON: { "questions": [...] }`:`Create a COMPREHENSIVE and DEEPLY EDUCATIONAL study worksheet about "${h}" in ${d} for a ${m} student at ${E.toUpperCase()} difficulty.${g?` Focus on: ${g}.`:""}${F}

${v?"CRITICAL: The student uploaded their own notes/materials. Your worksheet must be built ENTIRELY from the content in those notes. Every concept, vocabulary term, step, and practice question must come directly from the uploaded content. Do not add outside information. Cover every major topic and subtopic mentioned in the notes thoroughly.":`Build a thorough, curriculum-aligned worksheet that covers "${h}" in depth — not a surface-level overview. Include specific facts, formulas, processes, and examples a student would actually need to know for an exam.`}
...
Return JSON with this exact structure:
{
  "worksheet": {
    ...
    "summary": {
      ...
      "quickCheck": ["True or False: [specific statement about ${h}]", "Fill in the blank: [specific sentence about ${h}]", "Quick question: [something testable from the worksheet]"]
    },
    "practiceQuestions": [
      {
        "id": 1,
...
}`;let I=((await t.chat.completions.create({model:"gpt-4o",messages:[{role:"system",content:D},{role:"user",content:O}],temperature:.7,max_tokens:6e3})).choices[0].message.content??"{}").replace(/```json|```/g,"").trim();I=I.replace(/[\u0000-\u001F\u007F-\u009F]/g," ").replace(/\n/g," ").trim();try{s=JSON.parse(I)}catch(e){I=I.replace(/\\(?!["\\/bfnrtu])/g,"\\\\");try{s=JSON.parse(I)}catch(e){throw console.error("JSON parse failed:",I.substring(0,500)),Error("Failed to parse AI response. Please try again.")}}if(b)return p.NextResponse.json({sessionId:"retry",outputType:f,content:s});let{data:N,error:L}=await i.from("sessions").insert({user_id:o.id,subject:d,grade:m,topic:h,focus:g||null,output_type:f,content:s,difficulty:E}).select("id").single();if(L)throw L;return A.isPremium||await (0,u.BL)(i,o.id,x,A.usedBonus),p.NextResponse.json({sessionId:N.id,outputType:f})}catch(e){return console.error("Generate error:",e),p.NextResponse.json({error:e.message||"Generation failed"},{status:500})}}let h=new o.AppRouteRouteModule({definition:{kind:r.x.APP_ROUTE,page:"/api/generate/route",pathname:"/api/generate",filename:"route",bundlePath:"app/api/generate/route"},resolvedPagePath:"/Users/aysesamanci/projcet-2/src/app/api/generate/route.ts",nextConfigOutput:"",userland:i}),{requestAsyncStorage:g,staticGenerationAsyncStorage:f,serverHooks:y}=h,w="/api/generate/route";function v(){return(0,n.patchFetch)({serverHooks:y,staticGenerationAsyncStorage:f})}}};var t=require("../../../webpack-runtime.js");t.C(e);var s=e=>t(t.s=e),i=t.X(0,[9276,8456,3452,5972,4214,4067],()=>s(16880));module.exports=i})();