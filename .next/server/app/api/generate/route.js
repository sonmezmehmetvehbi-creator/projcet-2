"use strict";(()=>{var e={};e.id=8290,e.ids=[8290],e.modules={72934:e=>{e.exports=require("next/dist/client/components/action-async-storage.external.js")},54580:e=>{e.exports=require("next/dist/client/components/request-async-storage.external.js")},45869:e=>{e.exports=require("next/dist/client/components/static-generation-async-storage.external.js")},20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},92048:e=>{e.exports=require("fs")},32615:e=>{e.exports=require("http")},35240:e=>{e.exports=require("https")},55315:e=>{e.exports=require("path")},68621:e=>{e.exports=require("punycode")},76162:e=>{e.exports=require("stream")},17360:e=>{e.exports=require("url")},21764:e=>{e.exports=require("util")},6162:e=>{e.exports=require("worker_threads")},71568:e=>{e.exports=require("zlib")},87561:e=>{e.exports=require("node:fs")},84492:e=>{e.exports=require("node:stream")},72477:e=>{e.exports=require("node:stream/web")},43527:(e,t,s)=>{s.r(t),s.d(t,{originalPathname:()=>f,patchFetch:()=>g,requestAsyncStorage:()=>p,routeModule:()=>d,serverHooks:()=>m,staticGenerationAsyncStorage:()=>h});var r={};s.r(r),s.d(r,{POST:()=>u});var o=s(49303),i=s(88716),n=s(60670),a=s(6943),c=s(87070);let l=new(s(54214)).ZP({apiKey:process.env.OPENAI_API_KEY});async function u(e){try{let t;let s=await (0,a.f)(),{data:{user:r}}=await s.auth.getUser();if(!r)return c.NextResponse.json({error:"Unauthorized"},{status:401});let{subject:o,grade:i,topic:n,focus:u,outputType:d,questionCount:p,questionTypes:h,uploadedText:m,isRetry:f,difficulty:g}=await e.json(),{data:y}=await s.from("profiles").select("is_premium").eq("id",r.id).single();if(!y?.is_premium){let e=new Date().toISOString().split("T")[0],{data:t}=await s.from("daily_usage").select("questions, worksheets").eq("user_id",r.id).eq("date",e).single();if(("questions"===d?t?.questions??0:t?.worksheets??0)>=2)return c.NextResponse.json({error:"daily_limit_reached"},{status:429})}let w={mathematics:`
...
- Distractors: plausible-sounding but factually incorrect locations, statistics, or cause-effect relationships.`},b=o?.toLowerCase().trim(),v=["math","mathematics","calculus","algebra","pre-calculus","statistics","geometry","trigonometry"].some(e=>b?.includes(e)),q=w[b]||(v?w.mathematics:""),A=g||"medium",x=`You are AceForge, an expert educational content creator with deep knowledge of standardized exams, curriculum standards, and pedagogical best practices.
...
SUBJECT-SPECIFIC RULES FOR ${o?.toUpperCase()}:
${q||`
...
${m}
---
`:"",C=h?.includes("mc"),S=h?.includes("fr"),R="";R="questions"===d?`Generate ${p} ${A.toUpperCase()} difficulty study questions about "${n}" in ${o} for a ${i} student.${u?` Focus specifically on: ${u}.`:""}${k}
...
Ensure questions span different aspects of "${n}" — do not repeat the same concept.

Return JSON: { "questions": [...] }`:`Create a COMPREHENSIVE and DEEPLY EDUCATIONAL study worksheet about "${n}" in ${o} for a ${i} student at ${A.toUpperCase()} difficulty.${u?` Focus on: ${u}.`:""}${k}

${m?"CRITICAL: The student uploaded their own notes/materials. Your worksheet must be built ENTIRELY from the content in those notes. Every concept, vocabulary term, step, and practice question must come directly from the uploaded content. Do not add outside information. Cover every major topic and subtopic mentioned in the notes thoroughly.":`Build a thorough, curriculum-aligned worksheet that covers "${n}" in depth — not a surface-level overview. Include specific facts, formulas, processes, and examples a student would actually need to know for an exam.`}
...
Return JSON with this exact structure:
{
  "worksheet": {
    ...
    "summary": {
      ...
      "quickCheck": ["True or False: [specific statement about ${n}]", "Fill in the blank: [specific sentence about ${n}]", "Quick question: [something testable from the worksheet]"]
    },
    "practiceQuestions": [
      {
        "id": 1,
