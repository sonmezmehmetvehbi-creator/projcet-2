"use strict";(()=>{var e={};e.id=8290,e.ids=[8290],e.modules={72934:e=>{e.exports=require("next/dist/client/components/action-async-storage.external.js")},54580:e=>{e.exports=require("next/dist/client/components/request-async-storage.external.js")},45869:e=>{e.exports=require("next/dist/client/components/static-generation-async-storage.external.js")},20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},92048:e=>{e.exports=require("fs")},32615:e=>{e.exports=require("http")},35240:e=>{e.exports=require("https")},55315:e=>{e.exports=require("path")},68621:e=>{e.exports=require("punycode")},76162:e=>{e.exports=require("stream")},17360:e=>{e.exports=require("url")},21764:e=>{e.exports=require("util")},6162:e=>{e.exports=require("worker_threads")},71568:e=>{e.exports=require("zlib")},87561:e=>{e.exports=require("node:fs")},84492:e=>{e.exports=require("node:stream")},72477:e=>{e.exports=require("node:stream/web")},16880:(e,t,s)=>{s.r(t),s.d(t,{originalPathname:()=>f,patchFetch:()=>y,requestAsyncStorage:()=>m,routeModule:()=>d,serverHooks:()=>g,staticGenerationAsyncStorage:()=>h});var i={};s.r(i),s.d(i,{POST:()=>p});var o=s(49303),r=s(88716),n=s(60670),a=s(6943),c=s(26830),u=s(87070),l=s(54214);async function p(e){let t=new l.ZP({apiKey:process.env.OPENAI_API_KEY});try{let s;let i=await (0,a.f)(),{data:{user:o}}=await i.auth.getUser();if(!o)return u.NextResponse.json({error:"Unauthorized"},{status:401});let{subject:r,grade:n,topic:l,focus:p,outputType:d,questionCount:m,questionTypes:h,uploadedText:g,isRetry:f,difficulty:y}=await e.json(),w="questions"===d?"questions":"worksheets",v=await (0,c.$o)(i,o.id,w);if(!v.allowed)return u.NextResponse.json({error:"Daily limit reached",limitReached:!0,isPremium:v.isPremium,bonusRemaining:v.bonus},{status:429});let b={mathematics:`
...
- Distractors: plausible-sounding but factually incorrect locations, statistics, or cause-effect relationships.`},q=r?.toLowerCase().trim(),x=["math","mathematics","calculus","algebra","pre-calculus","statistics","geometry","trigonometry"].some(e=>q?.includes(e)),A=b[q]||(x?b.mathematics:""),k=y||"medium",C=`You are AceForge, an expert educational content creator with deep knowledge of standardized exams, curriculum standards, and pedagogical best practices.
...
- Distractors are plausible but unambiguously wrong.`,T=g?`
...
${g}
---
`:"",D=h?.includes("mc"),F=h?.includes("fr"),S="";S="questions"===d?`Generate ${m} ${k.toUpperCase()} difficulty study questions about "${l}" in ${r} for a ${n} student.${p?` Focus specifically on: ${p}.`:""}${T}

Include ${D&&F?"a mix of multiple choice and free response":F?"ONLY free response — do not include any multiple choice questions":"ONLY multiple choice — do not include any free response questions"} questions. This is strictly enforced — if the type says ONLY, do not include any other type.
...
Return JSON: { "questions": [...] }`:`Create a COMPREHENSIVE and DEEPLY EDUCATIONAL study worksheet about "${l}" in ${r} for a ${n} student at ${k.toUpperCase()} difficulty.${p?` Focus on: ${p}.`:""}${T}

${g?"CRITICAL: The student uploaded their own notes/materials. Your worksheet must be built ENTIRELY from the content in those notes. Every concept, vocabulary term, step, and practice question must come directly from the uploaded content. Do not add outside information. Cover every major topic and subtopic mentioned in the notes thoroughly.":`Build a thorough, curriculum-aligned worksheet that covers "${l}" in depth — not a surface-level overview. Include specific facts, formulas, processes, and examples a student would actually need to know for an exam.`}
...
}`;let E=((await t.chat.completions.create({model:"gpt-4o",messages:[{role:"system",content:C},{role:"user",content:S}],temperature:.7,max_tokens:6e3})).choices[0].message.content??"{}").replace(/```json|```/g,"").trim();E=E.replace(/[\u0000-\u001F\u007F-\u009F]/g," ").replace(/\n/g," ").trim();try{s=JSON.parse(E)}catch(e){E=E.replace(/\\(?!["\\/bfnrtu])/g,"\\\\");try{s=JSON.parse(E)}catch(e){throw console.error("JSON parse failed:",E.substring(0,500)),Error("Failed to parse AI response. Please try again.")}}if(f)return u.NextResponse.json({sessionId:"retry",outputType:d,content:s});let{data:P,error:R}=await i.from("sessions").insert({user_id:o.id,subject:r,grade:n,topic:l,focus:p||null,output_type:d,content:s,difficulty:k}).select("id").single();if(R)throw R;return v.isPremium||await (0,c.BL)(i,o.id,w,v.usedBonus),u.NextResponse.json({sessionId:P.id,outputType:d})}catch(e){return console.error("Generate error:",e),u.NextResponse.json({error:e.message||"Generation failed"},{status:500})}}let d=new o.AppRouteRouteModule({definition:{kind:r.x.APP_ROUTE,page:"/api/generate/route",pathname:"/api/generate",filename:"route",bundlePath:"app/api/generate/route"},resolvedPagePath:"/Users/aysesamanci/projcet-2/src/app/api/generate/route.ts",nextConfigOutput:"",userland:i}),{requestAsyncStorage:m,staticGenerationAsyncStorage:h,serverHooks:g}=d,f="/api/generate/route";function y(){return(0,n.patchFetch)({serverHooks:g,staticGenerationAsyncStorage:h})}}};var t=require("../../../webpack-runtime.js");t.C(e);var s=e=>t(t.s=e),i=t.X(0,[9276,8456,3452,5972,4214,199],()=>s(16880));module.exports=i})();