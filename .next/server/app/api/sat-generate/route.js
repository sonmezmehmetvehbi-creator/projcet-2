"use strict";(()=>{var e={};e.id=341,e.ids=[341],e.modules={72934:e=>{e.exports=require("next/dist/client/components/action-async-storage.external.js")},54580:e=>{e.exports=require("next/dist/client/components/request-async-storage.external.js")},45869:e=>{e.exports=require("next/dist/client/components/static-generation-async-storage.external.js")},20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},92048:e=>{e.exports=require("fs")},32615:e=>{e.exports=require("http")},35240:e=>{e.exports=require("https")},55315:e=>{e.exports=require("path")},68621:e=>{e.exports=require("punycode")},76162:e=>{e.exports=require("stream")},17360:e=>{e.exports=require("url")},21764:e=>{e.exports=require("util")},6162:e=>{e.exports=require("worker_threads")},71568:e=>{e.exports=require("zlib")},87561:e=>{e.exports=require("node:fs")},84492:e=>{e.exports=require("node:stream")},72477:e=>{e.exports=require("node:stream/web")},58534:(e,t,r)=>{r.r(t),r.d(t,{originalPathname:()=>x,patchFetch:()=>w,requestAsyncStorage:()=>g,routeModule:()=>h,serverHooks:()=>y,staticGenerationAsyncStorage:()=>f});var s={};r.r(s),r.d(s,{POST:()=>m});var o=r(49303),a=r(88716),i=r(60670),n=r(6943),c=r(3370),l=r(26830),u=r(57100),p=r(87070),d=r(54214);async function m(e){let t=new d.ZP({apiKey:process.env.OPENAI_API_KEY});try{let r;let s=await (0,n.f)(),{data:{user:o}}=await s.auth.getUser();if(!o)return p.NextResponse.json({error:"Unauthorized"},{status:401});let a=(0,c.createClient)(process.env.NEXT_PUBLIC_SUPABASE_URL,process.env.SUPABASE_SERVICE_ROLE_KEY),i=await (0,u.h)(a,o.id,"generation");if(i)return p.NextResponse.json({error:"generation_banned",reason:i.reason},{status:403});let{subject:d,module:m,questionCount:h,difficulty:g,topic:f}=await e.json();console.log("SAT generate received:",{subject:d,module:m,topic:f,difficulty:g,questionCount:h});let y=await (0,l.$o)(s,o.id,"sat");if(!y.allowed)return p.NextResponse.json({error:"Daily limit reached",limitReached:!0,isPremium:y.isPremium,bonusRemaining:y.bonus},{status:429});let x="math_no_calc"===m||"math_calc"===m,w="math_calc"===m,q={easy:"Easy (College Board difficulty 1-2): straightforward, single-concept, one clear path to the answer",medium:"Medium (College Board difficulty 3): requires applying a concept or interpreting information, 1-2 steps",hard:"Hard (College Board difficulty 4-5): multi-step reasoning, synthesis of multiple concepts, sophisticated distractors"},b=`You are an expert SAT question writer with 15+ years of experience writing for College Board. You have deep knowledge of the SAT format, question types, difficulty calibration, and distractor construction.
...
DIFFICULTY: ${q[g]??q.medium}

${x?`
SAT MATH RULES:
- Calculator allowed: ${w?"YES":"NO — questions must be solvable by hand in ~90 seconds"}
...
FORMATTING: Always respond in valid JSON only — no markdown, no preamble, no backticks.`,A="";if(A=x?`Generate ${h} SAT Math questions for the ${w?"Calculator":"No-Calculator"} module at ${g} difficulty.
...
Return JSON: { "questions": [...] }`:`Generate ${h} SAT Reading and Writing questions at ${g} difficulty.
...
Return JSON: { "questions": [...] }`,"string"==typeof f&&f.trim().toLowerCase().startsWith("mixed")){let e=x?w?"Heart of Algebra, Passport to Advanced Math, Problem Solving & Data Analysis, Geometry & Trigonometry":"Heart of Algebra, Passport to Advanced Math, Problem Solving & Data Analysis, Geometry & Trigonometry (no-calculator questions solvable by hand)":"Command of Evidence, Words in Context, Text Structure, Rhetorical Synthesis, Standard English";A+=`

Generate questions covering ALL topics within this SAT section randomly mixed. Do not focus on any single topic — vary the question topics throughout the set. Cover: ${e}.`}else f&&(A+=`

Focus every question specifically on this SAT skill/topic: ${f}.`);let S=((await t.chat.completions.create({model:"gpt-4o",messages:[{role:"system",content:b},{role:"user",content:A}],temperature:.7,max_tokens:16e3,response_format:{type:"json_object"}})).choices[0].message.content??"{}").replace(/```json|```/g,"").trim();S=S.replace(/[\u0000-\u001F\u007F-\u009F]/g," ").trim(),console.log("Raw AI response length:",S.length),console.log("Raw AI response start:",S.substring(0,200));try{r=JSON.parse(S)}catch{let e=S.replace(/\\(?!["\\/bfnrtu])/g,"\\\\");try{r=JSON.parse(e)}catch{let e=S.match(/\{[\s\S]*\}/);if(e)try{r=JSON.parse(e[0])}catch{}if(!r)throw Error("Failed to parse AI response. Please try again.")}}let{data:v,error:T}=await s.from("sessions").insert({user_id:o.id,subject:"SAT",grade:"college",topic:"math_no_calc"===m?"SAT Math (No Calculator)":"math_calc"===m?"SAT Math (Calculator)":"SAT Reading & Writing",output_type:"questions",content:r,difficulty:g,is_sat:!0,sat_module:m}).select("id").single();if(T)throw T;return y.isPremium||await (0,l.BL)(s,o.id,"sat",y.usedBonus),p.NextResponse.json({sessionId:v.id})}catch(e){return console.error("SAT generate error:",e),console.error("SAT generate error details:",e.message,e.stack),p.NextResponse.json({error:e.message||"Generation failed"},{status:500})}}let h=new o.AppRouteRouteModule({definition:{kind:a.x.APP_ROUTE,page:"/api/sat-generate/route",pathname:"/api/sat-generate",filename:"route",bundlePath:"app/api/sat-generate/route"},resolvedPagePath:"/Users/aysesamanci/projcet-2/src/app/api/sat-generate/route.ts",nextConfigOutput:"",userland:s}),{requestAsyncStorage:g,staticGenerationAsyncStorage:f,serverHooks:y}=h,x="/api/sat-generate/route";function w(){return(0,i.patchFetch)({serverHooks:y,staticGenerationAsyncStorage:f})}}};var t=require("../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),s=t.X(0,[9276,8456,3452,5972,4214,4067],()=>r(58534));module.exports=s})();