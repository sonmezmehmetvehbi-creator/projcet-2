"use strict";(()=>{var e={};e.id=7951,e.ids=[7951],e.modules={20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},56090:(e,t,o)=>{o.r(t),o.d(t,{originalPathname:()=>h,patchFetch:()=>_,requestAsyncStorage:()=>u,routeModule:()=>m,serverHooks:()=>f,staticGenerationAsyncStorage:()=>c});var r={};o.r(r),o.d(r,{GET:()=>g,dynamic:()=>l});var s=o(49303),i=o(88716),n=o(60670),a=o(3370),p=o(87070),d=o(82591);let l="force-dynamic";async function g(e){let t=new d.Resend(process.env.RESEND_API_KEY);if(e.headers.get("authorization")!==`Bearer ${process.env.CRON_SECRET}`)return p.NextResponse.json({error:"Unauthorized"},{status:401});let o=(0,a.createClient)(process.env.NEXT_PUBLIC_SUPABASE_URL,process.env.SUPABASE_SERVICE_ROLE_KEY),r=new Date,s=new Date(r.getTime()+9e5),i=new Date(r.getTime()+864e5),{data:n}=await o.from("tutoring_sessions").select("*, tutor_profiles(display_name, user_id), profiles!tutoring_sessions_student_id_fkey(email, display_name)").eq("status","confirmed").eq("reminder_15min_sent",!1).gte("scheduled_at",new Date(s.getTime()-3e5).toISOString()).lte("scheduled_at",new Date(s.getTime()+3e5).toISOString()),{data:l}=await o.from("tutoring_sessions").select("*, tutor_profiles(display_name, user_id), profiles!tutoring_sessions_student_id_fkey(email, display_name)").eq("status","confirmed").eq("reminder_24hr_sent",!1).gte("scheduled_at",new Date(i.getTime()-3e5).toISOString()).lte("scheduled_at",new Date(i.getTime()+3e5).toISOString()),g=0;for(let e of n??[])try{let{data:r}=await o.from("profiles").select("email").eq("id",e.tutor_profiles?.user_id).single(),s=new Date(e.scheduled_at).toLocaleString(),i=e.meet_link;await t.emails.send({from:"AceForge <noreply@aceforge.app>",to:e.profiles?.email,subject:"⏰ Your tutoring session starts in 15 minutes!",html:`
        ...
        `}),r?.email&&await t.emails.send({from:"AceForge <noreply@aceforge.app>",to:r.email,subject:"⏰ Your tutoring session starts in 15 minutes!",html:`
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
              <h2 style="color:#22550e">⏰ Session Starting Soon!</h2>
          ...
          `}),await o.from("tutoring_sessions").update({reminder_15min_sent:!0}).eq("id",e.id),g+=2}catch(e){console.error(e)}for(let e of l??[])try{let{data:r}=await o.from("profiles").select("email").eq("id",e.tutor_profiles?.user_id).single(),s=new Date(e.scheduled_at).toLocaleString();await t.emails.send({from:"AceForge <noreply@aceforge.app>",to:e.profiles?.email,subject:"\uD83D\uDCC5 Reminder: Tutoring session tomorrow",html:`
          <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
        ...
        `}),r?.email&&await t.emails.send({from:"AceForge <noreply@aceforge.app>",to:r.email,subject:"\uD83D\uDCC5 Reminder: Tutoring session tomorrow",html:`
            <div style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:24px">
              <h2 style="color:#22550e">📅 Session Reminder</h2>
          ...
          `}),await o.from("tutoring_sessions").update({reminder_24hr_sent:!0}).eq("id",e.id),g+=2}catch(e){console.error(e)}return p.NextResponse.json({success:!0,emailsSent:g})}let m=new s.AppRouteRouteModule({definition:{kind:i.x.APP_ROUTE,page:"/api/cron/send-reminders/route",pathname:"/api/cron/send-reminders",filename:"route",bundlePath:"app/api/cron/send-reminders/route"},resolvedPagePath:"/Users/aysesamanci/projcet-2/src/app/api/cron/send-reminders/route.ts",nextConfigOutput:"",userland:r}),{requestAsyncStorage:u,staticGenerationAsyncStorage:c,serverHooks:f}=m,h="/api/cron/send-reminders/route";function _(){return(0,n.patchFetch)({serverHooks:f,staticGenerationAsyncStorage:c})}}};var t=require("../../../../webpack-runtime.js");t.C(e);var o=e=>t(t.s=e),r=t.X(0,[9276,8456,5972,2591],()=>o(56090));module.exports=r})();